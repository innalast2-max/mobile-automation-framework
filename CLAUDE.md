...
Кожна задача — окрема гілка `feature/<назва>` (або `fix/<назва>`). Назва гілки завжди фігурує в задачі до код-рев'ю. Коміт(и) → push → PR → рев'ю → виправлення → merge. Без прямих комітів у `main`.

## 9. CI/CD

GitHub Actions: build + мобільні тести (профіль `-Pmobile`, за замовчуванням мобільні тести виключені з `mvn test`) → Allure-звіт на GitHub Pages per-PR. Червоний smoke — блокує merge.
